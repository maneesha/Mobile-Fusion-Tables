...
customInfoboxHtml: "<div style='color:black'>  {{#if isListView}} {{row.name}} {{else}} <a href= {{ row.website }} target = '_blank'> {{row.name}} </a> {{/if}} </div> {{#if row.closed }}CLOSED FOR SEASON{{/if}} <br>  {{row.day}} {{row.start_time}} to {{row.end_time}}. <br> <div style = 'white-space:pre-wrap'>{{row.at_place}}</div> {{#if isListView}} <div style='white-space:pre-wrap; color:#484848;'>Vendors: {{row.vendors}} </div> {{/if}}",
